...
Vertical Scaling :- In this case, the same server resources are increased like RAM, memory, CPU, and other hardware parts.
Horizontal Scaling :- In this case, multiple servers are created for handling the client requests.

So industry standard mostly uses horizontal scaling because it provides better performance and handles more traffic.

In my current project UniConnect web application, we used vertical scaling the server.

For handling multiple client requests, multiple servers are created and managed using load balancing.

On the frontend side, we used NGINX as a reverse proxy, and with the help of NGINX it manages the client requests.

On the backend side also, NGINX is configured, and inside the configuration we used the Round Robin algorithm. So round robin wise, requests hit different servers and responses are returned to the client.
...
First request goes to the first server
Second request goes to the second server
Third request goes to the third server
Again, if the fourth request occurs, it goes to the first server

So in this way, requests are properly distributed and the complete load does not come on a single server. This is the concept of the Round Robin algorithm.
...
->1.Event Emitter -> they are emmiting the events 
->2.Event Listener  -> they are listening the emmitting event 
->3.Event handler -> they are handling the event using the callback function.

const EventEmitter = require('event');
const eventEmitter = new EventEmitter();

eventEmitter.on('eventOccured',(req,res)=>{
	try {
		console.log('Handling Events..!');
	}catch(){ 
		console.log('Faild to Handling the events..!');
	}
});
...
POST → Used to create/send new data to the server.
PUT → Used to update all the data completely.
...
->It has many functions to handle paths, for example:
	path.parse() → Get file name, extension, and folder
	path.join() → Combine paths
	path.basename() → Get only the file name
In short:
...
// Module Wrapper Function example

// First "circular braces" scope
(function() {
    const name = "Vishal";
    function greet() {
    ...
    greet(); // Output: Hello Vishal
})();

// Second "circular braces" scope
(function() {
    const name = "NodeJS";
    function greet() {
...
function newShow()
{
	console.log('New Function are occured..!');
};
...
wait(1000,"Step Firest ..!")
    .then(()=>wait(2000,"Second Step..!"))
    .then(()=>wait(3000,"Third Step...!"))
    .then(()=>wait(4000,"Fourth Step..!"))
    .then(()=>console.log("Completed All Step..!"));
...
async function callWait(){
    await wait(1000,"Vishal Nimbarte");
    await wait(2000,"Vivek Moyaje");
    await wait(3000,"Karan Moyaje");
...
-> Every CPU core works like a thread.
...
-> Once they are completed, they are stored one by one in the callback queue.
-> The main work of the event loop is to continuously check whether the call stack is empty.
-> If the call stack is empty, the event loop moves tasks from the callback queue to the call stack for execution.
...
-> 1.Timers → Timers contain callbacks like setTimeout and setInterval.
-> 2.Pending Callbacks → These contain pending callbacks, which means operations are performed and processed.
-> 3.Idle / Prepare → These phases work internally and are not handled by the developer; they are handled internally by Node.js.
-> 4.Poll → This phase handles I/O callbacks.
...
->Streams are often used with packages like fs (file system), http, or zlib for file-related work, network requests, or compression.
---------------------------------------------------------------------------------------------------------------
...
-> Scenario: Most of the time, the same output is requested by multiple users.
-> The first time, the code executes normally and generates the response.
-> Next time, the same request comes, so the cached data from memory or temp.json/Redis is returned, avoiding full execution again.
...
-> If data is found, it is returned directly from Redis (very fast).
-> If data is not found, it is fetched from the database.
...
🔀 Frontend Side (Nginx Reverse Proxy)
	On the frontend side, I used Nginx reverse proxy.
	In the nginx.config file, I set multiple server paths.
	When multiple clients send requests, Nginx handles the request traffic and divides the requests to different servers.
	So every server gets a part of the traffic, and this scenario is frontend side load balancing.
⚙️ Backend Side (Node.js Cluster Module)
	On the backend side, Node.js  server by default uses single core.
	With the help of the cluster module, we can increase the processes.
...
-> Cluster module is used to handling the cores of system.
   ...
   With the help of cluster module, we create multiple processes using the system cores.
	...
	🔧 Example Scenario 
	Suppose the system contains 4 cores.
	So cluster will create 4 processes — each process runs on one core.
	When multiple client requests are incoming, the main thread will divide the request to these cores.
	These processes will work and return the data.

	🔁 How It Handles Requests
	Each process works like a worker thread.
	When request comes, the main thread sends it to a worker.
...
--> Bcrypt module for encryption → For password storage, I use bcrypt so plain text is not saved.
--> SHA-256 or AES → For extra security, I can use SHA-256 hashing or AES encryption for sensitive data.
--> Middleware implement → Middleware is used for authentication, validation, and request security checks.
--> HTTPS instead of HTTP → For secure communication, I always use HTTPS so data is encrypted in transfer.
...
->Cluster allows Node.js to:
	...
	Share the same port
	Distribute requests between workers

👉 This is basic backend load balancing.

🔹 How Cluster Works (Very Simple)
	Multiple Client Requests
			↓
	OS / Node.js Load Balancer
			↓
	Worker 1   Worker 2   Worker 3   Worker 4 ]]] there creation is deppend on system logical cores 
...
const promise_3 = new Promise((resolve, reject) => {
  return reject("Promise 3 Rejected..!");
});
...
These services work independently and communicate with each other using RabbitMQ for asynchronous messaging.

👉 Monlithic Architecture :- 
	Monolithic architecture means the entire application is built and developed as a single unit.
	If one part of the application goes down or an error occurs, the entire application goes down.

➡️ Example :- An e-commerce application where login, product listing, cart, payment, and orders are all built and deployed as one single Node.js application.

	If the payment module crashes
	The entire application goes down
	Even login and product pages stop working

👉 Microservice Architecture :- 
  	Microservices architecture means the application is divided into small, independent services that are built and developed separately.
	They are loosely coupled and not directly dependent on each other, but they communicate using APIs.

➡️ Example :- Imagine an e-commerce application:
	...
	In this case Each service is independent:
...
➡️ Message Broker (RabbitMQ / Kafka):
Message broker is mostly used for communication between services and it is a standard way.
In my old project, I used RabbitMQ, and in my current project, I am using Kafka.

➡️ WebSocket:
WebSocket is also used for communication between two or more services. In this case, one service emits events and another service gets the events using the .emit() and .on() functions.
...
In this case, showA() is executed first, and showB() and showC() wait until showA() execution is completed.
After that, showB() is executed, and then showC() is executed.
This is the process of synchronous programming.

➡️ Asynchronous Programming Language:
...
➡️require → CommonJS (Node.js)
➡️import → ES Modules (modern JavaScript)
...
Real Time Senario :- 
➡️WhatsApp chat – messages appear instantly
➡️Live notifications – like alerts on social media or apps
➡️Live stock updates or dashboards – data updates without refreshing
➡️Online gaming – real-time multiplayer interactions

➡️ Working of Socket :- 
	Connect the socket → establish connection between client and server.
	Emit events → send data from client to server or server to client.
...
✅ Code-First Approach
	...
	This happens because we use an ORM like Sequelize, TypeORM, or Mongoose

Example:
Code → ORM → Database (auto-created)

✅ Database-First Approach
	First, we create the database manually
	We use raw SQL queries to create tables
	Then we write the application code
	In this case, the database is NOT created automatically

Example:
Database → Code (manually mapped)
...
->express-rate-limit is used for handling unwanted requests so our Node.js server doesn’t crash.

✅Scenario:-
->If we create a Node server and do not handle rate limiting, multiple unwanted API calls can hit the server.
->This loads the server and can crash it.

✅Solution:-
->express-rate-limit handles requests from the same IP hitting the same API multiple times.
->It stops the excessive requests, preventing server overload.
->This is also called a DDoS (Distributed Denial of Service) scenario.
...
Example :- 
	...
	console.log(`Request from IP: ${ip}`); // log IP
	res.send(`API working! Your IP: ${ip}`);
	});
...
A JWT contains three main parts:-
	Header – contain which type request, mobile through or browser through
	Payload – user info
	Signature – secure key set

Example :- 

const jwt = require('jsonwebtoken');
const secret_key = 'Vishal@123';
const userInfo = {
  id: 1,
  username: 'vishal_nimbarte',
  role: 'USER'
};
...
→ The Node.js server contains a routes file, which checks which route and request type is called.
→ When the route matches, the corresponding controller file is executed.
→ The controller calls the service file (business logic).
→ The service file performs database operations or calls third-party services.
→ The service returns the response to the controller.
...
→ import works with static imports and supports tree-shaking.
...
→ find() :- 
	...
	and return the first matching element other return undefined values 
→ filter() :- 
	Used to filter data from an array
	Returns a new array with all matching elements
	If no match is found, it returns an empty array


const arr = [12,23,34,45,46,67];
...
bcrypt.hash('Vishal@123', 10, (err, hash) => {
  if (err) throw err;
  console.log(hash);
});
---------------------------------------------------------------------------------------------------------------
...
function regularFunction(){
    console.log('Regular Function Data..!');
    console.log();
}
...
->reduce method are used to they are return the single value to perform the operation on the array they are used to agregation regarding work on this array are need to the single value so we can used the reducer.
...
-> Also use SHA-256 algorithm, AES, and optimization techniques."
	bcrypt = password hashing (most common use case)
...
-> With the help of the require keyword, that custom module is used in the entire application.
	...
	const myFunction = () => "Hello from module!";
...
async function simpleWork(){
    console.log("Work..!");           
    await processWork();              // Pauses 2 seconds
...
function newFunction(callback){ --> Higher Order Function 
	callback();
};

newFunction(show);//callback function
------------------------------------------------Expressjs------------------------------------------------------
...
Answer:- 
	...
	So, event loop and thread pool help Node.js to handle non-blocking I/O.

	So, in every JavaScript file in Node.js, it contains two types of functions:
	synchronous and asynchronous.

	So, synchronous functions are executed one by one and stored in the call stack.
	So, this call stack executes in LIFO or FILO order, that means Last In First Out or First In Last Out.

	But asynchronous operations take more time because they request another server,
	call the database, like that type of operation, and they take more time.

	So, when this task is completed, it is stored in the callback queue.
	So, when everything process is completed, then the main work starts for the event loop.

	So, this event loop contains 6 main phases, like idle, prepare, timers, like that.

	So, the event loop continuously checks whether the call stack is empty or not.
	In case the call stack is empty, the callback queue contains tasks one by one priority-wise,
	and the callback is passed to the call stack.

	So, this callback is executed by the call stack,
	and with the help of the callback function, they return the response.
...
Answer:
	...
	So, for handling multiple requests at the same time, inside the Node.js application we use the cluster module, PM2, like that.

	So, the cluster module creates worker processes, and they get the CPU cores.
	That means our system contains logical processors like 4, 8, or 10, so that number of worker processes is created and used by the main thread to pass the tasks.

	So, in case multiple clients send multiple requests at the same time, the main thread divides every request to these worker processes.
	And this operation is performed by the worker processes, means system cores, and with the help of the callback function, they return the response.

	And if you want to scale the Node.js server, multiple Node.js servers are created, and they are configured with the help of NGINX.
...
Answer:
	...
	So, we should avoid CPU-intensive work and use I/O-intensive work in Node.js.
...
Answer:
	...
	So, the main thread can perform other tasks or event loop work.
	In this case, they do not block the event loop and do not affect the performance of the Node.js application.

	We can also use the cluster module for handling CPU-intensive work.
	That means one worker process is provided for performing the CPU-intensive work.

	Another way is to create a separate service in another programming language which is better for CPU-intensive work.
...
main.js 
	...
	console.log("Main thread continues other work");
...
Answer :- 
	...
	They are not dependent on each other, so in case one service is down, it does not affect another service.

	And API Gateway is the entry point of the client request.
	So, when multiple client requests come to the server, they first reach the API Gateway.

	API Gateway contains request validation, authentication, authorization, rate limiting, logging, and load balancing.

	And it does not contain business logic, and it does not have a database.

	So, it is the middle part between the client request and microservices, and with the help of this, it manages which service is called.

-----------------------------------------------------------------------------------------------------------------------
| Microservice                                    | API Gateway                                                       |
| ----------------------------------------------- | ----------------------------------------------------------------- |
| Each and every service is independent           | Entry point for client requests                                   |
| Developed independently                         | Handles incoming requests                                         |
| Deployed independently                          | Routes requests to services                                       |
| Each service has its own database               | Does not have a database                                          |
| One service down does not affect other services | Acts as middle layer                                              |
| Contains business logic                         | Does not contain business logic     	                              |
| Services communicate with each other            | Manages which service is called                                   |
...
Answer :- 
	...
	So, in the Node.js memory model, there are two types of memory:

	Static memory

	Heap memory

	Static memory stores local variables, primitive data types, and function calls.
	The call stack stores the data in FILO or LIFO order.

	Heap memory stores arrays, objects, and strings.
	Heap memory is dynamic memory, and it gets the size dynamically.
...
Expiry	Automatic	Manual cleanup
JWT best practices:
•	Use refresh tokens
•	HttpOnly cookies
•	Rotate tokens
...
•	Application layer — services, orchestrates logic
•	Infrastructure layer — DB, messaging, APIs
...
ES Modules use import and export keywords, which are part of modern JavaScript.
CommonJS modules are loaded synchronously, while ES Modules are loaded asynchronously.
...
Buffers are used when we want to handle a small amount of file data directly in memory. Buffers store raw binary data and are useful for tasks like reading small files, working with binary protocols, or manipulating data before writing.

Aspect							Streams										Buffers
Definition						Handle data chunk by chunk					Store raw binary data in memory
Use case						Large files or continuous data				Small data or binary manipulation
Memory usage					Efficient, low memory footprint				Consumes memory directly
Performance						Better for I/O-intensive tasks				Suitable for quick, small operations
...
-> Interceptors occur before the request reaches the server logic. They can add things like access tokens, modify the client request or response, and also perform validation.
...
Focus on I/O-intensive work instead of CPU-heavy tasks, since Node.js  is best for non-blocking I/O.
...
Answer:
	...
	Client calls refresh endpoint → sends the refresh token.
	Server validates refresh token → if valid, issues a new access token (sometimes also a new refresh token).
	Client stores the new token → in localStorage / sessionStorage.
	Client retries the original request → now with updated token.
	Server responds successfully → user gets the response.


//Real World Senario :- 

	When the client logs in, at that time two tokens are generated:
	access token and refresh token.

	The access token expiry time is less, like 15 or 20 minutes, and the
	refresh token expiry time is more, like 7 days or as per requirement.

	When the access token expires, in that case the client hits the refresh token API.
	The refresh token is validated, and if it is valid then the server returns a new access token.

	This new access token is again valid for 15 to 20 minutes.
	This process continues until the refresh token expiry time ends.

	When the refresh token expires, then the refresh token is not valid,
	and the system logs out the user and the user has to login again.
